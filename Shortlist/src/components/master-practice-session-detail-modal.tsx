...
type MasterPracticeSessionDetailModalProps = {
  open: boolean;
  loading: boolean;
...
export function MasterPracticeSessionDetailModal({
  open,
  loading,
  session,
  onClose,
  onDelete,
  deleteBusy = false,
}: MasterPracticeSessionDetailModalProps) {
  ...
  return (
    <MasterModal
      ...
      subtitle={
        session
          ? `${candidateLabel} · ${new Date(session.createdAt).toLocaleString()}`
